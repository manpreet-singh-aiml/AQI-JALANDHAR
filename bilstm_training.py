...
# ---------------------------------------------------------------------
# 4. BiLSTM + Attention model definition
# ---------------------------------------------------------------------
def create_bilstm_model():
    ...
    # Attention mechanism — dynamically weights timestep importance
    ...
    model = Model(inputs=inp, outputs=out, name="BiLSTM_Model")
    model.compile(
...
# ---------------------------------------------------------------------
print("\n=== BiLSTM RESULTS (mean +/- std over 5 seeds) ===")
print(f"\nNormalized:")
...
# ---------------------------------------------------------------------
# Paired t-test: LSTM vs BiLSTM (two-tailed, n=5)
# Loads per-seed metrics saved by lstm_training.py
# Run lstm_training.py first before running this file
# ---------------------------------------------------------------------
try:
    lstm_rmse_seeds = np.load("lstm_rmse_seeds.npy")
    lstm_mae_seeds  = np.load("lstm_mae_seeds.npy")

    t_rmse, p_rmse = stats.ttest_rel(lstm_rmse_seeds,
                                      np.array(all_rmse_norm))
    t_mae,  p_mae  = stats.ttest_rel(lstm_mae_seeds,
                                      np.array(all_mae_norm))

    print("\n=== Paired t-test: LSTM vs BiLSTM (two-tailed, n=5) ===")
    print(f"\nRMSE:")
    print(f"  t-statistic : {t_rmse:.4f}")
    print(f"  p-value     : {p_rmse:.4f}")
    print(f"  Significant : {'Yes (p < 0.01)' if p_rmse < 0.01 else 'No'}")
    print(f"\nMAE:")
    print(f"  t-statistic : {t_mae:.4f}")
    print(f"  p-value     : {p_mae:.4f}")
    print(f"  Significant : {'Yes (p < 0.01)' if p_mae < 0.01 else 'No'}")

except FileNotFoundError:
    print("\n[WARNING] lstm_rmse_seeds.npy not found.")
    print("Run lstm_training.py first to generate paired t-test results.")


# ---------------------------------------------------------------------
# Save BiLSTM per-seed metrics
# ---------------------------------------------------------------------
np.save("bilstm_rmse_seeds.npy", np.array(all_rmse_norm))
np.save("bilstm_mae_seeds.npy",  np.array(all_mae_norm))
...
plt.title("BiLSTM Training and Validation Loss Curves")
plt.xlabel("Epoch")
plt.ylabel("MSE Loss")
plt.legend()
plt.grid()
...
plt.title("BiLSTM AQI Time Series Prediction")
